...
list<T>::list(size_t num, const T &t) : numElements(num), pHead(nullptr), pTail(nullptr) {
    for (size_t i = 0; i < num; i++) {
        Node *newNode = new Node(t); // Create a new node with the given data

        if (!pHead) {
            // If the list is empty, set both pHead and pTail to the new node.
            pHead = newNode;
            pTail = newNode;
        } else {
...
list <T>& list <T> :: operator = (list <T> && rhs)
{
    if (this != &rhs) {
            ...
            // Transfer the pointers and state from rhs to this
            pHead = rhs.pHead;
            pTail = rhs.pTail;
            numElements = rhs.numElements;
...
//    Node* nextNode = nodeToRemove->pNext; // Get the next node.
//    Node* prevNode = nodeToRemove->pPrev; // Get the previous node.
//
//    if (nextNode != nullptr) {
